...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static void zipFiles(String srcDirName, String fileName,
		...
		try {
			ZipOutputStream zouts = new ZipOutputStream(byteArrayOutputStream);
	 ...
	 * @param srcDirName 压缩的根目录
	 * @param fileName   根目录下的待压缩的文件名或文件夹名，其中*或""表示跟目录下的全部文件
	 * @return 返回流 ZipOutputStream 必须在其他地方使用之前关闭
	 */
	public static InputStream zipFiles(String srcDirName, String fileName) {
		// 判断目录是否存在
		if (srcDirName == null) {
			log.debug("文件压缩失败，目录 " + srcDirName + " 不存在!");
			return null;
...
//	}
	...
	public static void traverseFolder(String resourcePath){
			...
			if (files.length == 0){
				...
				for (File file2 : files) {
					if (file2.isDirectory()) {
						...
						// TODO: 2018/4/25 查找同名称的文件
						String target = findFiles(file2.getName(), "D:\\new4\\party-core");
						if (null == target){
							System.err.println("新增的文件" + file2.getAbsolutePath());
						}
						else {
							copyFileCover(file2.getAbsolutePath(), target, true);
						}
					}
				}
			}
	...
	public static String findFiles(String name, String path){
		...
		if(files.length==0){
			...
			for (File file2 : files) {
				//文件夹 继续往下查找
				if (file2.isDirectory()) {
					String childName = findFiles(name, file2.getAbsolutePath());
					if (null != childName){
						return childName;
					}
				}
				//比较名称
					...
					if (file2.getName().equals(name) && !file2.getName().equals("pom.xml")){
